...
import static clov3r.oneit_server.error.errorcode.CommonErrorCode.REQUEST_ERROR;
import static clov3r.oneit_server.error.errorcode.CustomErrorCode.GIFTBOX_NOT_FOUND;
import static clov3r.oneit_server.error.errorcode.CustomErrorCode.NOT_EXIST_EMOJI;
import static clov3r.oneit_server.error.errorcode.CustomErrorCode.NOT_EXIST_INQUIRY;
import static clov3r.oneit_server.error.errorcode.CustomErrorCode.NOT_EXIST_INQUIRY_PRODUCT;
import static clov3r.oneit_server.error.errorcode.CustomErrorCode.NOT_MANAGER_OF_GIFTBOX;
import static clov3r.oneit_server.error.errorcode.CustomErrorCode.NOT_PARTICIPANT_OF_GIFTBOX;
...
public class InquiryController {
  ...
  public ResponseEntity<String> addEmoji(
    ...
    if (productEmojiList.stream().anyMatch(productEmoji -> productRepository.findById(productEmoji.getProductIdx()) == null)) {
      throw new BaseExceptionV2(PRODUCT_NOT_FOUND);
    }
    // check if all emoji of productEmojiList exists
    if (productEmojiList.stream().anyMatch(productEmoji -> !inquiryRepository.existEmojiById(productEmoji.getEmojiIdx()))) {
    ...
    if (productEmojiList.stream().anyMatch(productEmoji -> !inquiryRepository.existInquiryProduct(inquiryIdx, productEmoji.getProductIdx()))) {
  ...
  @Tag(name = "물어보기 API", description = "물어보기 API 목록")
  @Operation(summary = "물어보기 결과 조회(이모지 포함)", description = "물어보기 내용을 조회합니다.(각 제품별 이모지 포함)")
